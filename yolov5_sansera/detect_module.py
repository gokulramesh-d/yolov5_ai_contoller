...
def detector_get_crop(opt ,im0, class_name , names, padding = 0 ,img_size = 640 ,stride = 32 , model= detector , device = device ):
    ...
    # NMS
    pred = non_max_suppression(pred, conf_thres = opt.crop_conf, iou_thres = opt.crop_iou, classes = None, agnostic_nms = False, max_det=1000)
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                if names[c] == opt.crop_class :
                    crop = get_crop(xyxy, imc, gain=1.02, pad=10, square=False, BGR=False, save=True)

                # save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)
    return crop

def detector_get_inference(opt ,im0, class_name , names, padding = 0 ,img_size = 640 ,stride = 32 , model= detector , device = device ):
    # img0 = cv2.imread(path)
    predictions = []
    img = image_preprocess(im0 , img_size = img_size ,stride = stride)
    # img = torch.from_numpy(img).to(device)
